...
<svelte:head>
  <title>Ciudad · Total Soccer Manager</title>
</svelte:head>
...
<article class="prose max-w-5xl mx-auto py-6">
  <header class="flex items-baseline justify-between flex-wrap gap-4 mb-6 not-prose">
    <div>
      <h1 class="text-2xl font-bold">Ciudad de {data.derived?.clubCity ?? 'mi club'}</h1>
      {#if data.derived}
  ...
  {:else}
    ...
    <section class="not-prose">
      ...
      <PixiCanvas
        ...
        height={540}
        onTileClick={(coord) => {
          // The stadium occupies the 4×4 tile cluster centered on (32,32)
          // — clicking any of those tiles opens the close-up reformas view.
          const isStadiumTile =
            coord.tileX >= 30 && coord.tileX <= 33 &&
            coord.tileY >= 30 && coord.tileY <= 33;
          if (isStadiumTile) {
            goto('/stadium');
          }
        }}

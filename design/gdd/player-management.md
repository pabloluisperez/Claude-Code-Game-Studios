...
3. **OQ-PM-03**: ~~¿Cómo se presenta el mercado de fichajes?~~ **RESUELTO 2026-05-24**: `scouting-market.md` (v1.1) define 4 visibility tiers (T0/T1/T2/T3) controlled by `scouting_network_level` + `Scout Director` staff role. Pool size por nivel: L1=48 AI players visibles, L5=80. Free agents siempre todos. Player UI = filtros + saved searches + scout actions. Ver `scouting-market.md §3.1-3.2 + F1 + F5`.

4. **OQ-PM-04**: ~~¿Clubs IA con gestión de plantilla?~~ **RESUELTO 2026-05-24 (v1.1)**: `scouting-market.md §3.5 + F6` implementa AI club rotation mini-loop ejecutado en cada `transfer_window_open` event. Cada AI club: (1) marca for_sale candidates (age > 30 con prob 50%, low morale con prob 15%), (2) ejecuta buy decisions hasta `AI_TRANSFER_BUDGET_PCT × balance`, (3) promociona youth para llegar a `TARGET_ROSTER_SIZE`. Determinista vía seeded RNG (`worldSeed + windowId + clubId`). Reemplaza el stub MVP. Spec completa en `scouting-market.md`.

...
| OQ-HUD-10 | **Domain-language formatting de métricas** (post-vertical-slice 2026-05-18 — Pablo's playtest). Las métricas con rango [0,100] del cascade-engine NO deben mostrarse al jugador como números crudos sino como labels de fútbol: `team_fitness → "Muy mala/Mala/Regular/Buena/Excelente"`, `fan_momentum → "En crisis/Desencantada/Inquieta/Neutra/Animada/En llamas"`, `injury_risk → "Bajo/Medio/Alto/Crítico"`, `match_performance_index → "Para olvidar/Mal partido/Partido gris/Buen partido/Partidazo"`. `fan_attendance` además debe mostrarse como **cantidad absoluta** ("900 personas") **+ qualifier** ("Poco lleno"). La traducción vive en `src/web/src/lib/format.ts` (slice) y debe portarse a producción. Los buckets deben revisarse con el game-designer para alinear thresholds con cascadas relevantes (ej. <20 fan_momentum = "En crisis" coincide con BLOCKING threshold de cascade-engine.md). | UX designer + game-designer | **Resuelta de raíz por playtest** — formalizar buckets en `/ux-design` antes del sprint UI. |
| OQ-HUD-11 | **Match event animations + pixel art feedback** (post-vertical-slice 2026-05-18 — Pablo's design vision). Los eventos del partido en directo necesitan VISUAL FEEDBACK además del texto: (a) **GOL** → pequeña animación pixel art del balón chocando contra la red + confetti para gol propio (slice usa emoji confetti CSS; producción usa sprite animation); (b) **LESIÓN** → jugador en camilla siendo retirado a la banda; (c) **EXPULSIÓN por agresión** → animación de jugador entrando en plancha + tarjeta roja; (d) **TARJETA AMARILLA** → árbitro mostrando tarjeta; (e) **PENALTI** (post-MVP en match-sim) → señalización del lanzamiento. Pablo asume que estas animaciones son **MVP, no v1.2+**. **Importante**: aunque la decisión arquitectónica MVP es DOM-only (scope-mvp.md §3.2), estas animaciones se pueden implementar como sprite/GIF/Lottie embebidos en `<img>` tags dentro de los modals del HUD — NO requieren PixiJS canvas. La línea entre "DOM con pixel art assets" y "canvas isométrico" se mantiene. | art-director + UX designer + Pablo | Abierta — abrir nuevo `/ux-design match-events` que catalogue cada tipo de evento + la animación correspondiente. Coordinar con art-bible.md (Lived-In Pixel) para tile size 32×16 y paleta. |
| OQ-HUD-12 | **Dramatic event pacing + skip control** (post-vertical-slice 2026-05-18 — Pablo's playtest). El flow del partido en directo necesita una pausa anticipatoria (teaser banner: "⚡ ¡Algo está pasando!" / "👀 Atento al área..." — neutro, no revela el resultado) ANTES del modal del evento. El modal del gol debe sostener ~10s con countdown visible y botón "Continuar →" para acelerar. El close button (✕) en la esquina del modal es un escape inmediato. El backdrop del modal NO debe difuminar el confetti (`backdrop-filter: blur` rompe la celebración). Estas reglas de pacing van en el spec de `MatchPauseEvent` UI y aplican a todos los `varReview` / `goal` / `injury_pause` events. | UX designer | **Resuelta de raíz por playtest** — formalizar en `/ux-design match-events`. |
| OQ-HUD-13 | **Live match playback timing + fast-forward control** (post-vertical-slice 2026-05-18 — Pablo's feedback). El slice usó 100ms/tick (~9s por partido completo) — demasiado rápido para sentir el ritmo de un partido. Producción debe usar **~1 segundo = 1 minuto in-game** (90s por partido completo, alineado con FM clásico y PC Fútbol). Necesita un **toggle de velocidad de reproducción** (×1, ×3, ×10) accesible durante el partido + **botón "Saltar al final"** que dispara el resto del partido server-side sin animación. Razones: (a) Pilar 4 (Calm Is The Tempo) — el ritmo natural permite leer eventos y disfrutar momentos dramáticos sin perderse el siguiente; (b) los modals dramáticos (10s, OQ-HUD-12) tienen impacto solo si entre eventos hay tiempo "vacío" que los enmarque; (c) el confetti + animaciones pixel-art (OQ-HUD-11) necesitan tempo. **Edge case**: el toggle de velocidad debe persistir entre matches (preferencia de sesión, localStorage). | UX designer + game-designer | **Resuelta de raíz por playtest** — formalizar en `/ux-design match-events` junto con OQ-HUD-11/12. |

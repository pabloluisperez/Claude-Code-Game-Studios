...
- **OQ-TV-02** ⚠️ **BREAKING CHANGE** (ver Dependencies): `TV_RIGHTS_SEGUNDA = 3 €K/sem` y `TV_RIGHTS_PRIMERA = 8 €K/sem` en `constants.ts`, y `getTVRightsWeekly()` en `league-system.md §F6`, deben eliminarse al implementar este sistema. El GDD es la autoridad — los valores correctos son los de F-TV1.
- **OQ-TV-03** ⚠️ **BREAKING CHANGE menor (R5)**: F-TV4 introduce `fan_loyalty × 0.005` como multiplicador de `fan_attendance` en cálculo de matchday revenue. El nodo `fan_attendance` (C8) sigue residiendo en cascade-engine, pero el cálculo de matchday revenue (probablemente `economy.md §F3`) ahora debe aplicar F-TV4. Decisión pendiente: ¿la multiplicación se aplica como nodo derivado en cascade-engine (nuevo C-fan-attendance-effective) o como modificador en economy.md §F3? Pertenece al cross-review tras la aprobación de tv-rights.md. Adicionalmente: `fan_loyalty` debe registrarse en `entities.yaml` como entidad cross-sistema antes de la implementación.
- **OQ-TV-04** 🔧 **ADR de implementación requerido (R6)**: Antes de iniciar código, el web-backend-specialist debe crear un ADR ("ADR: TV Rights implementation contract") que especifique: (1) tick pipeline como transacción DB atómica; (2) parse string→float de `corruption_exposure` en Drizzle; (3) unique constraint DB para `tv_auction`/`tv_midseason_offer`; (4) serialización week-38 tick + season_end en el mismo job BullMQ; (5) shape del `metadata.decisionPayload` de `tv_auction` y persistencia de `offer_id`; (6) schema de `tv_contract` y `fan_loyalty` (tabla, PK, tipos); (7) idempotency del advance worker en retry; (8) atomicidad de `POST /api/tv/sign` con event resolution. No son blockers del GDD — son blockers del sprint de implementación.
